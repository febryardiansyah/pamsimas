...
class _ScanResultScreenState extends State<ScanResultScreen> {
        ...
        child: BlocConsumer<GetUserByUidCubit, GetUserByUidState>(
          listener: (context, state) {
            if (state is GetUserByUidSuccess) {
              setState(() {
                _lastBillManualCtrl.text = state.data?.bill == null
                    ? ''
                    : state.data!.bill!.currentUsage!.toString();
                _lastUsage = state.data?.bill == null
                    ? null
                    : state.data!.bill!.currentUsage!;
                _lastBill = state.data?.bill == null
                    ? null
                    : state.data!.bill!.currentBill!;
              ...
              return SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Meteran Lalu : ${_data.bill == null ? '-' : _data.bill!.currentUsage}',
                            ),
                          ],
                        ),
                        // Spacer(),
                    ...
                    GestureDetector(
                      onTap: _inputMeterCtrl.text.isEmpty ||
                              _selectedMonth == null ||
                              _currentYear.text.isEmpty
                          ? null
                          : () {
  ...
  int _calculateBill() {
    ...
    _bill +=
        _additionalPrice.text.isEmpty ? 0 : int.parse(_additionalPrice.text);
    setState(() {
      _totalBill = _bill;
    });
    return _bill;
  ...
  void _showCalculationResult(int currentBill) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          '${Helper.formatCurrency(_totalBill)}',
        ...
        actions: [
          ElevatedButton(
            child: Text(
              'Lanjutkan',
              style: GoogleFonts.roboto(color: BaseColor.white),
            ),
            onPressed: () {
              Navigator.pop(context);
              context.read<InputUserBillCubit>().inputBill(
                    ...
                    year: _currentYear.text,
                    currentUsage: int.parse(_inputMeterCtrl.text),
